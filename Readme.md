...
- ANSWER: This helps keep the code clean and easier to follow. It organize routes into smaller pieces which makes the code more manageable and easy to understand.
...
- ANSWER: It tells the server to listen on a specific port. It first checks if there's a port number set in the environment variables.
- If there's no environment port set, it falls back to 8081. This is useful because, in production, the hosting service picks the port automatically, while in development, we can use a fixed port like 8081 to test things easily.
---
